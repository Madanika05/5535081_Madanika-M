...
int* maxSubarray(int arr_count, int* arr, int* result_count) {
    *result_count = 2;
    ...
    for (int i = 0; i < arr_count; i++) {
        ...
        if (arr[i] > best_subsequence) {
            best_subsequence = arr[i];
        }
    }

    if (found_positive) {
        best_subsequence = subsequence_sum;
    }

    result[0] = best_subarray;
    result[1] = best_subsequence;
