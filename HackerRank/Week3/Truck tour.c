...
int truckTour(int petrolpumps_rows, int petrolpumps_columns, int** petrolpumps) {
    (void)petrolpumps_columns;
    for (int start = 0; start < petrolpumps_rows; start++) {
        long fuel = 0;
        int count = 0;
        for (int step = 0; step < petrolpumps_rows; step++) {
            int idx = (start + step) % petrolpumps_rows;
            fuel += petrolpumps[idx][0];
            fuel -= petrolpumps[idx][1];
            if (fuel < 0) break;
            count++;
        }
        if (count == petrolpumps_rows) return start;
    }
    return -1;
}
