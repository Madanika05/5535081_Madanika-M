#include <assert.h>
#include <ctype.h>
#include <limits.h>
#include <math.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

char* readline();
char* ltrim(char*);
char* rtrim(char*);
char** split_string(char*);

int parse_int(char*);

static void generate_primes(int limit, int* primes, int* count) {
    bool* is_prime = calloc(limit + 1, sizeof(bool));
    for (int i = 2; i <= limit; i++) is_prime[i] = true;
    for (int p = 2; p * p <= limit; p++) {
        if (is_prime[p]) {
            for (int j = p * p; j <= limit; j += p) {
                is_prime[j] = false;
            }
        }
    }
    *count = 0;
    for (int i = 2; i <= limit; i++) {
        if (is_prime[i]) {
            primes[(*count)++] = i;
        }
    }
    free(is_prime);
}

int* waiter(int number_count, int* number, int q, int* result_count) {
    int* result = malloc(number_count * sizeof(int));
    *result_count = 0;
    int primes[1300], prime_count = 0;
    generate_primes(10000, primes, &prime_count);
    int* current = malloc(number_count * sizeof(int));
    int cur_size = number_count;
    for (int i = 0; i < number_count; i++) current[i] = number[i];
    for (int i = 0; i < q; i++) {
        int prime = primes[i];
        int* nextA = malloc(cur_size * sizeof(int));
        int nextA_size = 0;
        int* Bi = malloc(cur_size * sizeof(int));
        int Bi_size = 0;
        for (int j = cur_size - 1; j >= 0; j--) {
            if (current[j] % prime == 0) {
                Bi[Bi_size++] = current[j];
            } else {
                nextA[nextA_size++] = current[j];
            }
        }
        for (int j = Bi_size - 1; j >= 0; j--) {
            result[(*result_count)++] = Bi[j];
        }
        free(current);
        current = nextA;
        cur_size = nextA_size;
        free(Bi);
        if (cur_size == 0) break;
    }
    for (int j = cur_size - 1; j >= 0; j--) {
        result[(*result_count)++] = current[j];
    }
    free(current);
    return result;
}

int main() {
    char** first_multiple_input = split_string(rtrim(readline()));
    int number_count = parse_int(*(first_multiple_input + 0));
    int q = parse_int(*(first_multiple_input + 1));
    char** number_temp = split_string(rtrim(readline()));
    int* number = malloc(number_count * sizeof(int));
    for (int i = 0; i < number_count; i++) {
        int number_item = parse_int(*(number_temp + i));
        *(number + i) = number_item;
    }
    int result_count;
    int* result = waiter(number_count, number, q, &result_count);
    for (int i = 0; i < result_count; i++) {
        printf("%d", *(result + i));
        if (i != result_count - 1) printf("\n");
    }
    printf("\n");
    return 0;
}

char* readline() {
    size_t alloc_length = 1024;
    size_t data_length = 0;
    char* data = malloc(alloc_length);
    while (true) {
        char* cursor = data + data_length;
        char* line = fgets(cursor, alloc_length - data_length, stdin);
        if (!line) break;
        data_length += strlen(cursor);
        if (data_length < alloc_length - 1 || data[data_length - 1] == '\n') break;
        alloc_length <<= 1;
        data = realloc(data, alloc_length);
    }
    if (data[data_length - 1] == '\n') data[data_length - 1] = '\0';
    data = realloc(data, data_length);
    return data;
}

char* ltrim(char* str) {
    if (!str) return NULL;
    if (!*str) return str;
    while (*str != '\0' && isspace(*str)) str++;
    return str;
}

char* rtrim(char* str) {
    if (!str) return NULL;
    if (!*str) return str;
    char* end = str + strlen(str) - 1;
    while (end >= str && isspace(*end)) end--;
    *(end + 1) = '\0';
    return str;
}

char** split_string(char* str) {
    char** splits = NULL;
    char* token = strtok(str, " ");
    int spaces = 0;
    while (token) {
        splits = realloc(splits, sizeof(char*) * ++spaces);
        splits[spaces - 1] = token;
        token = strtok(NULL, " ");
    }
    return splits;
}

int parse_int(char* str) {
    return atoi(str);
}
