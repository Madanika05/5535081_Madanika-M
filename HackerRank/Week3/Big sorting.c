...
/*
 * Compare two numeric strings by their integer value without converting to integers.
 * Primary key: length; Secondary key: lexicographic order.
 */
static int cmp_numstr(const void *pa, const void *pb) {
...
char** bigSorting(int unsorted_count, char** unsorted, int* result_count) {
    qsort(unsorted, unsorted_count, sizeof(char*), cmp_numstr);
