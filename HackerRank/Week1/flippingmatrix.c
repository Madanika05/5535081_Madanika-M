...
int flippingMatrix(int matrix_rows, int matrix_columns, int** matrix) {
    ...
    for (int r = 0; r < n; r++) {
        for (int c = 0; c < n; c++) {
            ...
            int choices[4];
            choices[0] = matrix[r][c];
            choices[1] = matrix[r][matrix_columns - 1 - c];
            choices[2] = matrix[matrix_rows - 1 - r][c];
            choices[3] = matrix[matrix_rows - 1 - r][matrix_columns - 1 - c];

            int max_val = choices[0];
            for (int i = 1; i < 4; i++) {
                if (choices[i] > max_val) {
                    max_val = choices[i];
                }
            }
